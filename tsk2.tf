...
resource "aws_s3_bucket" "bckt_043" {
  bucket = "tsk1bucket"
  acl    = "public-read"
...
resource "aws_s3_bucket_object" "deployimage" {
	bucket = aws_s3_bucket.bckt_043.bucket
	key = "cloudtask1.jpg"
...
locals {
  ...
  image-url = "${aws_cloudfront_distribution.s3_distribution.domain_name}/${aws_s3_bucket_object.deployimage.key}"
}

resource "aws_cloudfront_distribution" "s3_distribution" {
  origin {
    domain_name = aws_s3_bucket.bckt_043.bucket_domain_name
